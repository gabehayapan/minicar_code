def ReadPWMPARAM(pwm):
    ...
    with open(path) as f:
        l = f.readlines()
        PWM_PARAM = ([int(l[2]), int(l[4]), int(l[6])],
                     [int(l[9]), int(l[11]), int(l[13])])
    return PWM_PARAM
...
def Accel(PWM_PARAM, pwm, time, Duty):
    ...
    if Duty > 0:
        ...
        pwm.set_pwm(13, 0, THROTTLE_STOPPED_PWM)
        time.sleep(0.01)
        throttle_pwm = int(THROTTLE_WTOPPED_PWM + (THROTTLE_REVERSE_PWM - THROTTLE_STOPPED_PWM) * abs(Duty) / 100)
        pwm.set_pwm(13, 0, throttle_pwm)
